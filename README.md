# OpenWrt / iStoreOS Cloudflared 一键安装管理脚本

[![GitHub](https://img.shields.io/github/license/hxzlplp7/openwrt-one-click-cloudflared)](LICENSE)

这是一个适用于 **OpenWrt** 和 **iStoreOS** 设备的 Cloudflare Tunnel (cloudflared) 一键安装与管理脚本。

## ⚠️ 重要说明

本脚本使用 **Cloudflare 远程管理隧道 (Remotely-Managed Tunnel)** 方式，这是 Cloudflare 推荐的新方式。

**您需要先在 Cloudflare 面板创建隧道，获取 Token 后再使用本脚本。**

> 💡 这种方式的优点是：隧道配置（域名绑定、路由规则等）全部在 Cloudflare 网页面板管理，无需在路由器上编辑配置文件。

## ✨ 功能特点

- 🔧 **自动检测架构** - 支持 amd64 (x86_64), arm64 (aarch64), arm (armv7) 架构
- 📦 **一键安装** - 自动下载官方二进制文件并安装依赖
- 🚀 **服务管理** - 通过 OpenWrt 标准的 PROCD 进行管理（启动/停止/重启/开机自启）
- 🎯 **交互式菜单** - 简单易用的中文管理界面
- 📖 **内置指南** - 详细的 Token 获取说明
- ⌨️ **命令行支持** - 支持命令行参数直接操作
- 📋 **日志查看** - 方便查看运行日志排查问题

## 📋 系统要求

- OpenWrt 18.06+ 或 iStoreOS
- 至少 50MB 可用存储空间
- 可访问 GitHub 的网络连接（用于下载二进制文件）
- Root 权限

## 🚀 快速开始

### 第一步：在 Cloudflare 创建隧道获取 Token

1. 打开 [Cloudflare Zero Trust 面板](https://one.dash.cloudflare.com/)
2. 登录您的 Cloudflare 账户
3. 在左侧菜单找到 **Networks** → **Tunnels**
4. 点击 **Create a tunnel** (创建隧道)
5. 选择 **Cloudflared** 作为连接器类型
6. 给隧道起一个名字 (例如: `openwrt-tunnel`)
7. 在 "Install and run a connector" 页面，找到类似这样的命令：
   ```
   cloudflared tunnel run --token eyJhIjoixxxxx...
   ```
8. 复制 `--token` 后面的那一长串字符（以 `eyJ` 开头的 Base64 编码字符串）

### 第二步：运行安装脚本

使用 SSH 连接到你的路由器，运行以下命令：

```bash
wget -O /tmp/cfd.sh https://raw.githubusercontent.com/hxzlplp7/openwrt-one-click-cloudflared/main/cloudflared_install.sh && sh /tmp/cfd.sh
```

### 第三步：配置并启动

1. 在菜单中选择 **1. 安装 Cloudflared**
2. 安装完成后，选择 **2. 配置 Token**
3. 粘贴第一步获取的 Token
4. 选择 **4. 启动服务**
5. 回到 Cloudflare 面板配置 **Public Hostname** 以暴露您的服务

### 快捷命令

安装成功后，可以直接在终端输入以下命令唤起管理菜单：

```bash
cfd
```

## 📖 使用流程图

```
┌─────────────────────────────────────────────────────────────┐
│                    Cloudflare Zero Trust 面板                │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────────┐  │
│  │ 创建 Tunnel │ -> │  获取 Token │ -> │ 配置 Public Host│  │
│  └─────────────┘    └─────────────┘    └─────────────────┘  │
└─────────────────────────────────────────────────────────────┘
                            │
                            ▼ Token
┌─────────────────────────────────────────────────────────────┐
│                     OpenWrt / iStoreOS                       │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────────┐  │
│  │ 安装脚本    │ -> │ 配置 Token  │ -> │ 启动服务        │  │
│  └─────────────┘    └─────────────┘    └─────────────────┘  │
└─────────────────────────────────────────────────────────────┘
```

## 📖 命令行用法

脚本支持命令行参数，方便自动化操作：

```bash
# 安装
cfd install

# 配置 Token
cfd token

# 显示获取 Token 的指南
cfd guide

# 启动服务
cfd start

# 停止服务
cfd stop

# 重启服务
cfd restart

# 查看状态
cfd status

# 查看日志
cfd logs

# 卸载
cfd uninstall

# 显示帮助
cfd help
```

## 📁 文件路径

| 文件 | 路径 |
|------|------|
| 二进制文件 | `/usr/bin/cloudflared` |
| 配置目录 | `/etc/cloudflared/` |
| Token 文件 | `/etc/cloudflared/token` |
| 启动脚本 | `/etc/init.d/cloudflared` |
| 快捷命令 | `/usr/bin/cfd` |

## ❓ 常见问题

### Q: 为什么需要 Token？不能直接登录吗？

A: Cloudflare Tunnel 有两种使用方式：
1. **本地管理隧道** - 需要运行 `cloudflared tunnel login` 在浏览器中登录
2. **远程管理隧道 (本脚本使用)** - 在 Cloudflare 面板创建隧道后获取 Token

由于 OpenWrt 路由器通常没有图形界面，无法方便地进行浏览器登录，因此本脚本采用 Token 方式。这也是 Cloudflare 推荐的方式，因为所有配置都可以在网页面板中管理。

### Q: Token 是什么样的？

A: Token 是一个以 `eyJ` 开头的长字符串（Base64 编码的 JSON），例如：
```
eyJhIjoiYWJjMTIzNDU2Nzg5MGRlZmdoaWprbG1ub3BxcnN0dXZ3eHl6IiwidCI6ImFiYzEyMzQ1Njc4OTBlZGVmZ2hpamtsbW5vcHFyc3R1dnd4IiwicyI6ImFiY2RlZmdoaWprbG1ub3BxcnN0dXZ3eHl6MTIzNDU2Nzg5MCJ9
```

### Q: 如何暴露我的服务？

A: 在 Cloudflare Zero Trust 面板中：
1. 进入 **Networks** → **Tunnels**
2. 点击您创建的隧道
3. 切换到 **Public Hostname** 标签
4. 点击 **Add a public hostname**
5. 配置域名和后端服务地址（例如 `http://192.168.1.1:80`）

### Q: 下载失败怎么办？

A: 如果无法连接 GitHub，可以：
- 使用代理/VPN
- 手动下载对应架构的 cloudflared 二进制文件，上传到 `/usr/bin/cloudflared`

下载地址：https://github.com/cloudflare/cloudflared/releases/latest

## ⚠️ 注意事项

- **网络问题**: 脚本默认从 GitHub 官方 Releases 下载二进制文件
- **内存占用**: Cloudflared 运行时内存占用在 30-100MB 左右
- **存储空间**: cloudflared 二进制文件约 40MB

## 🐛 问题排查

如果服务无法启动，可以查看日志：

```bash
# 通过脚本查看
cfd logs

# 或直接使用 logread
logread | grep cloudflared
```

## 📜 许可证

MIT License

## 🙏 致谢

- [Cloudflare](https://www.cloudflare.com/) - 提供免费的 Tunnel 服务
- [OpenWrt](https://openwrt.org/) - 开源路由器固件
